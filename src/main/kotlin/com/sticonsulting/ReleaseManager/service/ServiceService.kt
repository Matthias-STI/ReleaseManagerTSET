...
class ServiceService(private val serviceRepository: ServiceRepository) {
    ...
    private fun addDeployment(name: String, versionNr: Int): Int {
        ...
        val newServiceDeployment = ServiceEntity(name, versionNr, systemVersion)
    ...
    fun getServicesWithSystemVersion(systemVersion: Int): List<ServiceEntity> {
        return getAllWithSystemVersionSmallerEqualThan(systemVersion).groupBy { it.name }.entries.map { it.value.first() }
            .sortedBy { it.systemVersionNr }
    }
    ...
    private fun getAllWithSystemVersionSmallerEqualThan(systemVersion: Int): List<ServiceEntity> {
        return serviceRepository.findAll().filter { it.systemVersionNr <= systemVersion }
            .sortedByDescending { it.systemVersionNr }
    }
}
